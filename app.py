...
def generate_pdf(gene, expr, muts, drugs):
    ...
    pdf.cell(0, 10, "Expression Data", ln=True, fill=True)
    pdf.set_font("Arial", '', 11)
    if expr:
        for sample, value in expr.items():
            pdf.cell(0, 8, f"{sample}: {value}", ln=True)
    else:
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "Mutation Info", ln=True, fill=True)
    pdf.set_font("Arial", '', 11)
    if muts:
        for mut in muts:
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "Drug Matches", ln=True, fill=True)
    pdf.set_font("Arial", '', 11)
    if drugs:
        for drug in drugs:
